...
class NlpService
{
    ...
    public function extractIntent(string $rawQuery): array
    {
        ...
        try {
            ...
            $result = $this->parseResponse($content, $rawQuery);
    ...
    private function systemPrompt(): string
    {
        ...
        Output: {"FoodSearch": "chicken katsu", "FoodType": "chicken", "MaxPrice": 50000, "MaxDistance": 3000, "VisitTime": "now"}

        Input: "ayam geprek deket sini"
        Output: {"FoodSearch": "ayam geprek", "FoodType": "chicken", "MaxPrice": 0, "MaxDistance": 1000, "VisitTime": "now"}

        Input: "pengen ramen hangat malem ini"
        Output: {"FoodSearch": "ramen", "FoodType": "ramen", "MaxPrice": 0, "MaxDistance": 3000, "VisitTime": "night"}

        Input: "cari nasi padang murah sekitar sini"
        Output: {"FoodSearch": "nasi padang", "FoodType": "indonesian", "MaxPrice": 30000, "MaxDistance": 3000, "VisitTime": "now"}

        Input: "aku mau minum kopi susu deket sini"
        Output: {"FoodSearch": "kopi susu", "FoodType": "coffee", "MaxPrice": 0, "MaxDistance": 1000, "VisitTime": "now"}

        Input: "dinner romantis buat anniversary"
        Output: {"FoodSearch": "", "FoodType": "any", "MaxPrice": 0, "MaxDistance": 3000, "VisitTime": "evening"}

        Input: "I want to eat some dimsum"
        Output: {"FoodSearch": "dimsum", "FoodType": "chinese", ...}

        Input: "I am craving for some salt bread"
        Output: "FoodSearch": "salt bread", "FoodType": "any", ...}

        RULE: FoodSearch must contain the user's actual dish words.
        NEVER put a FoodType category into FoodSearch.
        NEVER put the literal "any" into FoodSearch - if you don't know, copy the user's noun phrase.

        Return exactly this shape:
        {"FoodSearch": "", "FoodType": "any", "MaxPrice": 0, "MaxDistance": 3000, "VisitTime": "now"}
        PROMPT;
    }

    private function parseResponse(string $content, string $rawQuery): array
    {
        ...
        $placeholders = ['', 'any', 'none', 'null', 'food', 'restaurant', 'anything'];
        if (in_array(strtolower($foodSearch), $placeholders, true)) {
            $foodSearch = $rawQuery;
        }
